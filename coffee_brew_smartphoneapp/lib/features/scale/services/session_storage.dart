...
class SessionStorage {
  ...
  static const String _recipesFileName = 'brew_recipes.json';
  static const String _currentRecipeIdFileName = 'current_recipe_id.txt';
  static const String _beanStocksFileName = 'bean_stocks.json';
  ...
  static Future<List<BrewRecipe>> loadRecipes() async {
    try {
      final dir = await _getDir();
      final file = File('${dir.path}/$_recipesFileName');
      if (!await file.exists()) return [];
      final text = await file.readAsString();
      if (text.trim().isEmpty) return [];
      final decoded = jsonDecode(text) as List;
      return decoded
          .map((e) => BrewRecipe.fromJson(Map<String, dynamic>.from(e)))
          .toList();
    } catch (_) {
      return [];
    }
  ...
  static Future<void> saveRecipes(List<BrewRecipe> recipes) async {
    final dir = await _getDir();
    final file = File('${dir.path}/$_recipesFileName');
    await file.writeAsString(jsonEncode(recipes.map((e) => e.toJson()).toList()));
  }

  static Future<String?> loadCurrentRecipeId() async {
    try {
      final dir = await _getDir();
      final file = File('${dir.path}/$_currentRecipeIdFileName');
  ...
  static Future<void> saveCurrentRecipeId(String? id) async {
    final dir = await _getDir();
    final file = File('${dir.path}/$_currentRecipeIdFileName');
  ...
  static Future<List<BeanStock>> loadBeanStocks() async {
    try {
      final dir = await _getDir();
      final file = File('${dir.path}/$_beanStocksFileName');
      if (!await file.exists()) return [];
      final text = await file.readAsString();
      if (text.trim().isEmpty) return [];
      final decoded = jsonDecode(text) as List;
      return decoded
          .map((e) => BeanStock.fromJson(Map<String, dynamic>.from(e)))
          .toList();
    } catch (_) {
      return [];
    }
  ...
  static Future<void> saveBeanStocks(List<BeanStock> stocks) async {
    final dir = await _getDir();
    final file = File('${dir.path}/$_beanStocksFileName');
    await file.writeAsString(
        jsonEncode(stocks.map((e) => e.toJson()).toList()));
  }
