...
class _SessionDetailScreenState extends State<SessionDetailScreen> {
  ...
  Future<void> _load() async {
    ...
    if (session != null) {
      ...
      _doseController.text =
          session.doseG == null ? '' : session.doseG!.toStringAsFixed(1);
      _tdsController.text =
          session.tdsPercent == null ? '' : session.tdsPercent!.toString();
    }
  ...
  Future<void> _exportCsv(CoffeeSession session) async {
    try {
      await CsvExportService.shareSessionPoints(session);
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                  'Dose: ${session.doseG == null ? "-" : "${session.doseG!.toStringAsFixed(1)} g"}'),
              Text(
                  'Ratio: ${session.brewRatio == null ? "-" : "1:${session.brewRatio!.toStringAsFixed(1)}"}'),
              Text(
                  'TDS: ${session.tdsPercent == null ? "-" : "${session.tdsPercent} %"} / EY: ${session.extractionYieldPercent == null ? "-" : "${session.extractionYieldPercent!.toStringAsFixed(1)} %"}'),
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            onPressed: session.points.isEmpty ? null : () => _exportCsv(session),
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            ...
                            Expanded(
                              child: Text(
                                'EY: ${session.extractionYieldPercent == null ? "-" : "${session.extractionYieldPercent!.toStringAsFixed(1)} %"}',
                              ),
                            ),
                          ],
                        ),
